...
- `FlashSelfAttention`（`src/models/flash_attention.py`）：
  - 单线性层投影得到 `Q/K/V`，重排为 `(batch, heads, seq_len, head_dim)`。
  - 支持 KV-cache：沿序列维拼接 `K/V` 并返回更新后的缓存。
  - 使用 `scaled_dot_product_attention(..., is_causal=True)` 触发 Flash 内核，随后经 `out_proj` 输出。
- `PythiaFlashModel`（`src/models/pythia_flash_model.py`）：
  - 加载 `EleutherAI/pythia-70m`，遍历每层注意力模块，替换为 Flash 适配器并复制 `query_key_value` 与 `dense` 权重。
  - 保持与 HF `generate()` 兼容的前向接口与缓存行为。
...
- `compute_ppl.py`：
  - 使用 teacher forcing 计算 `loss` 与 `ppl`，支持 WikiText/PG-19 两套评估。
